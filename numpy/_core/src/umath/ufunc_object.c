                   ...
                   int axis, PyArray_DTypeMeta *signature[])
{
    ...
    for (int i = 0; i < nout; i++) {
        ...
        /* If input and output overlap in memory, use iterator to sort out */
                 ...
                 PyArray_DTypeMeta *signature[])
{
    ...
    /*
     * Local, owned view of the outputs: `out_arrays` aliases the caller's
     * array, so we must not overwrite its entries.  `out[i]` holds our own
     * reference (or the array we allocate).
     */
    PyArrayObject *out[NPY_MAXARGS];
    for (int i = 0; i < nout; i++) {
        out[i] = out_arrays[i];
    ...
    /* Optional first-element copy (stream -> out_i) per output for casts. */
    NPY_cast_info copy_info[NPY_MAXARGS];
    for (int i = 0; i < nout; i++) {
        NPY_cast_info_init(&copy_info[i]);
    }
     ...
     * Descriptor layout produced by the resolver (2*nout+1 entries):
     *     [acc_0 .. acc_{nout-1}, stream, out_0 .. out_{nout-1}]
     * with acc_i == out_i.  For a single-output ufunc this collapses to the
     * classic [acc, stream, out] triplet.
     */
    PyArray_Descr *descrs[NPY_MAXARGS];
    PyArrayMethodObject *ufuncimpl = reducelike_promote_and_resolve(ufunc,
            arr, out, signature, NPY_TRUE, descrs, NPY_UNSAFE_CASTING,
            ...
            for (i = 0; i < ind_size; ++i) {
                ...
                for (int j = 0; j < nout; j++) {
                    ...
                    if (descrs[nout + 1 + j]->type_num == NPY_OBJECT) {
                        /* Incref before decref in case they alias. */
        ...
        PyObject *const *args, Py_ssize_t len_args, PyObject *kwnames, int operation)
{
    ...
    /*
     * `reduce`, `accumulate`, and `reduceat` all support multi-output ufuncs
     * whose resolved loop registers a dedicated reduction loop (see
     * `reducelike_promote_and_resolve_multi`).  A multi-output ufunc without
     * a reduction loop raises a `TypeError` later, at resolution time.
     */
