...
:exc:`ValueError` (or, for the reduce-like methods on a ufunc with more than
one output, a :exc:`TypeError`), unless the ufunc's loop implementation
registers a dedicated reduction loop, in which case
:meth:`~numpy.ufunc.reduce`, :meth:`~numpy.ufunc.accumulate`, and
:meth:`~numpy.ufunc.reduceat` all work for ufuncs with more than one
output. See
...
loop, :meth:`~numpy.ufunc.reduce`, :meth:`~numpy.ufunc.accumulate`, and
:meth:`~numpy.ufunc.reduceat` instead return a tuple with one array per
